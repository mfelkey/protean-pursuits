...
## Finance Team Standalone Team

Finance is now a **fully standalone team** with its own directory at `teams/finance-team/` and dedicated intake flow at `flows/finance_intake_flow.py`. It still runs as a logical sub-crew within the Dev team pipeline — after full planning (post UX Content Guide), before the build phase — but can also be invoked independently for any project. Advisory only — it **never auto-blocks the pipeline**. Fires checkpoint CP-4.5 on FSP completion.

Specialist agents remain at `templates/dev-team/agents/finance/`. The Finance Team orchestrator imports from there.

**Direct invocation:**

    python flows/finance_intake_flow.py --project parallaxedge \
        --brief "Full financial analysis" --save
...
**Invocation:**
    ...
    # Full crew via intake flow (standalone)
    python flows/finance_intake_flow.py --project myproject --brief "..." --save
    # Full crew via finance_flow (direct, any mode)
    python flows/finance_flow.py --mode full --project myproject --task "..."
    # Single-agent shortcut
    python flows/finance_flow.py --mode cost --task "Cost estimate for X"
    # Via pp_flow
    python flows/pp_flow.py --team finance --mode full --project myproject --task "..." --save
...
Project-agnostic domain specialists callable from any current or future Protean Pursuits project. Location: `agents/sme/` · Standalone team directory: `teams/sme-team/` · Intake via: `flows/sme_flow.py`

**⛔ Authorized callers only:** `pp_orchestrator`, `project_manager`, `strategy`, `legal`, `sme_flow`. Dev-team agents are explicitly blocked — a `PermissionError` is raised at runtime via `validate_caller()`. Attempting to call from an unauthorized agent raises immediately; there is no fallback.
...
| **VIDEO\_TOOL\_SELECTION** | Video team — Tool Analyst | Approve tool selection before any creative work or API calls begin. |
| **SCRIPT\_REVIEW** | Video team — Script Writer | Approve script before API calls or production work begins. |
| **VIDEO\_FINAL** | Video team — Compliance Reviewer | Final approval before any publish action. Nothing publishes without this gate. |
...
## 📦 ModuleNotFoundError / ImportError on intake flow

**What happened:** A flow cannot find its team orchestrator. Most common cause: the `teams/<team>-team/` directory is missing from `sys.path`, or the orchestrator file doesn't exist yet.

**Fix:** Verify the team directory exists and the orchestrator has a `run_*` function:

    ls teams/<team>-team/agents/orchestrator/orchestrator.py
    grep "^def run_" teams/<team>-team/agents/orchestrator/orchestrator.py

If missing, the team may need to be scaffolded. See `docs/agent_system_operating_manual.md` Section 9 for the add-a-new-agent pattern.

**Never hardcode `/home/<user>/` paths** in agent or flow files. All `sys.path` entries must use dynamic `Path(__file__).resolve()` resolution.
...
5. Set `sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent.parent.parent))` at the top — never hardcode a user home path
...
| SME Group | SME Orchestrator | `agents/sme/sme_orchestrator.py` · `teams/sme-team/agents/orchestrator/orchestrator.py` | T1 | Domain Intelligence Brief — authorized callers: pp_orchestrator, project_manager, strategy, legal, sme_flow |
...
| HR Team | HR Orchestrator | `teams/hr-team/agents/orchestrator/orchestrator.py` | T1 | Sequences HR crew |
