...
def run_frontend_implementation(context: dict, tip_path: str, uxd_path: str,
    ...
    # ── RAG: inject current knowledge (React/Next.js updates, accessibility) ──
    project_title = context.get("structured_spec", {}).get("title", "project")
    knowledge = get_knowledge_context(
        agent_role="Frontend Developer",
        task_summary=f"Frontend implementation for {project_title}",
    )
    ...
    fir_task = Task(
        description=f"""
You are the Frontend Developer for the following project. Using the upstream
documents below, produce a complete Frontend Implementation Report (FIR) with
working code for all key components and pages.
