"""
flows/legal_intake_flow.py

Protean Pursuits — Legal Team Intake Flow

Direct entry point to the Legal Team for post-initiation requests.
...
Legal Team Orchestrator. The orchestrator selects the appropriate run
mode and sequences agents based on the brief.

HITL gate: LEGAL_REVIEW fires on all modes. No document is finalised autonomously.
Legal is an authorized SME caller — SME invocations propagate caller='legal' automatically.

Run modes (selected automatically by Legal Team Orchestrator):
    contract    Draft contract
    review      Redlined document + risk memo
    ip          IP assessment, licensing framework
    privacy     Privacy analysis, GDPR/CCPA assessment
    regulatory  Compliance gap report
    employment  Employment agreement, contractor framework
...
Usage:
    ...
    python flows/legal_intake_flow.py \
        --brief "Draft NDA for a contractor engagement — Michigan law, 2-year term, IP assignment clause"

    # With existing project context
    python flows/legal_intake_flow.py \
        --project parallaxedge \
        --brief "Review our Terms of Service for GDPR compliance gaps"

    # Brief from file
    python flows/legal_intake_flow.py \
        --project parallaxedge \
        --brief-file briefs/legal_brief.txt \
        --save

Via pp_flow.py:
    python flows/pp_flow.py --team legal --mode full \
        --project parallaxedge --task "Draft NDA for a contractor engagement — Michigan law, 2-year term, IP assignment clause" --save
"""
...
def _import_orchestrator():
    """Import the Legal Team Orchestrator run function. Exits with clear message on failure."""
    try:
        from agents.legal.orchestrator.orchestrator import run_legal_orchestrator
        return run_legal_orchestrator
    except ImportError as e:
        sys.exit(
            f"[legal_intake_flow] ERROR: Cannot import Legal Team Orchestrator.\n"
            f"  Ensure agents/legal/orchestrator/orchestrator.py is on sys.path.\n"
            f"  sys.path includes /home/mfelkey/legal-team — verify team is installed.\n"
            f"  Original error: {e}"
        )
...
def _build_stub_context(project_name: str, brief: str) -> dict:
    ...
    The Legal Team Orchestrator fills in objectives and details from the brief.
    """
    return {
...
def _resolve_brief(args: argparse.Namespace) -> str:
    ...
    if args.brief_file:
        ...
        if not path.exists():
            sys.exit(f"[legal_intake_flow] ERROR: Brief file not found: {path}")
        return path.read_text(encoding="utf-8").strip()
    if args.brief:
        return args.brief.strip()
    sys.exit(
        "[legal_intake_flow] ERROR: A brief is required.\n"
...
def run_legal_intake(
    ...
    """
    Route a plain-English brief to the Legal Team Orchestrator.
    ...
    Args:
        ...
        save:            Write output to disk under output/<project>/legal/.
        non_interactive: Skip all interactive prompts (for scripted use).

    Returns:
        Legal Team Orchestrator output as a string.
    """
    # ── Brief enrichment ──────────────────────────────────────────────────────
    ...
    try:
        ...
        logger.warning(
            "Legal Team Orchestrator did not accept 'brief' kwarg — "
    ...
    print("\n" + "=" * 60)
    print(f"LEGAL TEAM ORCHESTRATOR OUTPUT — {project.upper()}")
    print("=" * 60)
    print(output_str)
    print("=" * 60 + "\n")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="legal_intake_flow.py",
        description=(
            "Protean Pursuits — Legal Team Intake Flow\n\n"
            "Direct entry point for post-initiation legal requests.\n"
            "Accepts a plain-English brief and routes to the Legal Team\n"
            ...
            "  python flows/legal_intake_flow.py \\\n"
            "      --project parallaxedge \\\n"
            "      --brief \"Review our Terms of Service for GDPR compliance gaps\" --save\n\n"
            "  # Brief from file\n"
            "  python flows/legal_intake_flow.py \\\n"
            "      --project parallaxedge \\\n"
            "      --brief-file briefs/legal_brief.txt --save"
        ),
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Save output to output/<project>/legal/intake_<timestamp>.md. "
