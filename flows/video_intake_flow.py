"""
flows/video_intake_flow.py

Protean Pursuits — Video Team Intake Flow

Direct entry point to the Video Team for post-initiation video requests.
...
Video Team Orchestrator. The orchestrator selects the appropriate run
mode and sequences agents based on the brief.

HITL gates fire automatically — no video deliverable is produced autonomously:
...
Run modes (selected automatically by Video Team Orchestrator):
...
Usage:
    ...
    python flows/video_intake_flow.py \
        --brief "60s TikTok ad for ParallaxEdge sports betting app launch — US market, Q3"

    # With existing project context
    python flows/video_intake_flow.py \
        --project parallaxedge \
        --brief "Animated explainer for the bet slip feature, 90s, YouTube"

    # Brief from file
    python flows/video_intake_flow.py \
        --project parallaxedge \
        --brief-file briefs/video_brief.txt \
        --save

Via pp_flow.py:
    python flows/pp_flow.py --team video --mode SHORT_FORM \
        --project parallaxedge --task "60s TikTok ad for ParallaxEdge launch" --save
"""
...
def _import_orchestrator():
    """Import the Video Team Orchestrator run function. Exits with clear message on failure."""
    try:
        from agents.orchestrator.orchestrator import run_video_orchestrator
        return run_video_orchestrator
...
def _build_stub_context(project_name: str, brief: str) -> dict:
    ...
    The Video Team Orchestrator fills in format, duration, and platform
...
def _resolve_brief(args: argparse.Namespace) -> str:
    ...
    if args.brief_file:
        ...
        if not path.exists():
            sys.exit(f"[video_intake_flow] ERROR: Brief file not found: {path}")
        return path.read_text(encoding="utf-8").strip()
    if args.brief:
        return args.brief.strip()
    sys.exit(
        "[video_intake_flow] ERROR: A brief is required.\n"
...
def run_video_intake(
    ...
    """
    Route a plain-English brief to the Video Team Orchestrator.

    The orchestrator selects the appropriate run mode based on the brief
    (format, platform, duration, output type) and sequences agents accordingly.
    HITL gates fire as normal — no deliverable is produced autonomously.

    Args:
        brief:           Plain-English description of the video request.
        context:         Project context dict (from load_context or stub).
        save:            Write output to disk under output/<project>/video/.
        non_interactive: Skip all interactive prompts (for scripted use).

    Returns:
        Video Team Orchestrator output as a string.
    """
    # ── Brief enrichment ──────────────────────────────────────────────────────
    ...
    try:
        ...
        logger.warning(
            "Video Team Orchestrator did not accept 'brief' kwarg — "
    ...
    print("\n" + "=" * 60)
    print(f"VIDEO TEAM ORCHESTRATOR OUTPUT — {project.upper()}")
    print("=" * 60)
    print(output_str)
    print("=" * 60 + "\n")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="video_intake_flow.py",
        description=(
            "Protean Pursuits — Video Team Intake Flow\n\n"
            "Direct entry point for post-initiation video requests.\n"
            "Accepts a plain-English brief and routes to the Video Team\n"
            "Orchestrator, which selects the appropriate run mode.\n\n"
            "HITL gates fire automatically — no deliverable is produced autonomously:\n"
            "  VIDEO_TOOL_SELECTION — after Tool Analyst\n"
            "  SCRIPT_REVIEW        — after Script Writer\n"
            "  VIDEO_FINAL          — after Compliance Reviewer\n\n"
            "Examples:\n"
            "  # Cold start\n"
            "  python flows/video_intake_flow.py \\\n"
            "      --brief \"60s TikTok ad for ParallaxEdge sports betting app launch\"\n\n"
            "  # Existing project\n"
            "  python flows/video_intake_flow.py \\\n"
            "      --project parallaxedge \\\n"
            "      --brief \"Animated explainer for the bet slip feature, 90s, YouTube\" --save\n\n"
            "  # Brief from file\n"
            "  python flows/video_intake_flow.py \\\n"
            "      --project parallaxedge \\\n"
            "      --brief-file briefs/video_brief.txt --save"
        ),
    ...
    brief_group.add_argument(
        "--brief",
        metavar="TEXT",
        help="Plain-English video brief.",
    )
    brief_group.add_argument(
        "--brief-file",
        dest="brief_file",
        metavar="PATH",
        help="Path to a text file containing the video brief.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Save output to output/<project>/video/intake_<timestamp>.md. "
