"""
flows/ds_intake_flow.py

Protean Pursuits — DS Team Intake Flow

Direct entry point to the DS Team for post-initiation requests.
...
DS Team Orchestrator. The orchestrator selects the appropriate run
mode and sequences agents based on the brief.

HITL gates fire on HITL_DS_REVIEW for full pipeline runs.

Run modes (selected automatically by DS Team Orchestrator):
    brief       Scoping brief — framing, questions, complexity classification
    evaluation  Data evaluation — quality assessment, gaps, readiness
    analysis    Full EDA and statistical analysis
    model       Model selection, training, evaluation
    pipeline    End-to-end DS pipeline including handoff package

Usage:
    # Cold start — no existing project
    python flows/ds_intake_flow.py \
        --brief "Run EDA and model pipeline for Q3 churn dataset — PostgreSQL source, 500k rows, binary classification"

    # With existing project context
    python flows/ds_intake_flow.py \
        --project parallaxedge \
        --brief "Run statistical analysis on our betting market feature data"

    # Brief from file
    python flows/ds_intake_flow.py \
        --project parallaxedge \
        --brief-file briefs/ds_brief.txt \
        --save

Via pp_flow.py:
    python flows/pp_flow.py --team ds --mode full \
        --project parallaxedge --task "Run EDA and model pipeline for Q3 churn dataset — PostgreSQL source, 500k rows, binary classification" --save
"""
...
def _import_orchestrator():
    """Import the DS Team Orchestrator run function. Exits with clear message on failure."""
    try:
        from agents.ds.ds_orchestrator import run_ds_orchestrator
        return run_ds_orchestrator
...
def _build_stub_context(project_name: str, brief: str) -> dict:
    ...
    The DS Team Orchestrator fills in objectives and details from the brief.
    """
    return {
...
def _resolve_brief(args: argparse.Namespace) -> str:
    ...
    if args.brief_file:
        ...
        if not path.exists():
            sys.exit(f"[ds_intake_flow] ERROR: Brief file not found: {path}")
        return path.read_text(encoding="utf-8").strip()
    if args.brief:
        return args.brief.strip()
    sys.exit(
        "[ds_intake_flow] ERROR: A brief is required.\n"
...
def run_ds_intake(
    ...
    """
    Route a plain-English brief to the DS Team Orchestrator.
    ...
    Args:
        ...
        save:            Write output to disk under output/<project>/ds/.
        non_interactive: Skip all interactive prompts (for scripted use).

    Returns:
        DS Team Orchestrator output as a string.
    """
    # ── Brief enrichment ──────────────────────────────────────────────────────
    ...
    logger.info(f"DS Team intake — project: {project}")
    ...
    try:
        ...
        logger.warning(
            "DS Team Orchestrator did not accept 'brief' kwarg — "
    ...
    print("\n" + "=" * 60)
    print(f"DS TEAM ORCHESTRATOR OUTPUT — {project.upper()}")
    print("=" * 60)
    print(output_str)
    print("=" * 60 + "\n")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ds_intake_flow.py",
        description=(
            "Protean Pursuits — DS Team Intake Flow\n\n"
            "Direct entry point for post-initiation ds requests.\n"
            "Accepts a plain-English brief and routes to the DS Team\n"
            ...
            "  python flows/ds_intake_flow.py \\\n"
            "      --project parallaxedge \\\n"
            "      --brief \"Run statistical analysis on our betting market feature data\" --save\n\n"
            "  # Brief from file\n"
            "  python flows/ds_intake_flow.py \\\n"
            "      --project parallaxedge \\\n"
            "      --brief-file briefs/ds_brief.txt --save"
        ),
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Save output to output/<project>/ds/intake_<timestamp>.md. "
